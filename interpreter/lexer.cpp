...
Lexer::Lexer(LanguageInfo& linfo)
	: languageInfo_(linfo)
	, offset_(0)
	, lineCount_(1)
{
...
bool Lexer::getToken(std::string& text, Token& token)
{
	if(!skipSpaces(text))
		return false;
...
void Lexer::checkCompoundStatement(unsigned int& position, std::string& text)
{
...
bool Lexer::skipSpaces(std::string& text)
{
	if(strchr(" \t\r\n",text[offset_])) {
...
bool Lexer::skipComments(std::string& text)
{
	if(text[offset_] == '#') {
...
bool Lexer::defTokenValue(std::string& text, std::string& value)
{
...
void Lexer::defTokenType(Token& token, std::string& value)
{
	std::vector<std::string>& compOperations = languageInfo_.comparisonOperators;
	map<string, Token::TokenType>& tocenDict = languageInfo_.tocenDict_;
	

	if(tocenDict.find(value) != tocenDict.end()) {
		token.setType(tocenDict[value]);
	}
...
bool Lexer::checkIsNumber(std::string& value)
{
...
void Lexer::verifyVariableName(std::string& name)
{
	bool isValid = true;
